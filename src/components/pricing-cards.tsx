...
import type { PricingFeature, PricingTierConfig } from "@/lib/pricing";
  ...
  return (
    <section
      className={cn(
        ...
        "overflow-hidden px-4 py-12 sm:py-16",
        sectionClassName
      )}
    >
      <div
        className={cn("mx-auto w-full max-w-7xl px-4", containerClassName)}
        {...props}
      >
        <div
          className={cn(
            "grid grid-cols-1 items-stretch gap-8 sm:grid-cols-2 lg:grid-cols-3",
            className
          )}
        >
          {tiers.map((tier, index) => (
            <Reveal key={tier.name} delay={index * 0.1} className="h-full">
              <PricingCard tier={tier} cardClassName={cardClassName} />
            </Reveal>
          ))}
        </div>
      </div>
...
function PricingCard({
  ...
  return (
    <div
      className={cn(
        "group relative h-full rounded-2xl transition-all duration-500",
        tier.highlight
    ...
    >
      <div className="flex h-full flex-col p-8 lg:p-10">
        ...
        {tier.variantSelector && (
          <div className="mt-6">{tier.variantSelector}</div>
        )}
        ...
        {tier.cta && (
          <div className="mt-8">
            <Button
              className={cn(
                "group/btn relative h-12 w-full transition-all duration-300",
                tier.highlight
                  ? "bg-white text-neutral-900 hover:bg-neutral-100"
                  : "bg-neutral-900 text-white hover:bg-neutral-800 dark:bg-white dark:text-neutral-900 dark:hover:bg-neutral-100"
              )}
              onClick={tier.cta.onClick}
              disabled={tier.cta.disabled || tier.cta.loading}
            >
              <span className="relative z-10 flex items-center justify-center gap-2 font-medium tracking-wide">
                {tier.cta.loading ? "Redirecting..." : tier.cta.text}
                {!tier.cta.loading && (
...
export function buildTierFromConfig(
